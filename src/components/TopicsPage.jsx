...
import TopicsPagination from "../functions/TopicsPagination"

function TopicsPage ({topicArticles, page, setPage}){
...
return (<main className="row">
            ...
            {topicArticles.length ?
                <li key={topicArticles[0].article_id}>
                    ...
                    <div style={{display: "flex", flexDirection: "row", justifyContent: "start"}}>
                      <ButtonGroup aria-label="radius button group" color="danger"  variant='solid'  sx={{ '--ButtonGroup-radius': '40px', mt: 3.9, ml: 2}}>
                        <Button className='' sx={{fontSize: "15px"}}>   {topicArticles[0].topic.slice(0,1).toUpperCase() + topicArticles[0].topic.substring(1)}</Button>
                        </ButtonGroup>
        ...
        <TopicsPagination page={page} setPage={setPage} />
        </section>
